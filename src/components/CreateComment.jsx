/* eslint-disable jsx-a11y/control-has-associated-label */
import React from 'react';
import Proptypes from 'prop-types';
import { useSelector } from 'react-redux';

function CreateComment({ onCommentChange, onCreateCommentSubmit, commentThread }) {
  const { avatar } = useSelector((states) => states.authUser.user);
  return (
    <div className="sticky -bottom-[17px] p-1 bg-white  mt-[60px]">
      <div className="flex gap-2">
        <img src={avatar} alt={avatar} className="rounded-full w-[30px] h-[30px]" />
        <form className="w-full" onSubmit={onCreateCommentSubmit}>
          <div className="bg-[#F0F2F5] rounded-xl p-2">
            {/* eslint-disable-next-line react/self-closing-comp */}
            <textarea value={commentThread} onChange={onCommentChange} placeholder="Type a comment..." className="bg-transparent focus:outline-none resize-none w-full h-[80px]">{commentThread}</textarea>
            <div className="flex justify-end">
              <button type="submit">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M9.51002 4.23001L18.07 8.51001C21.91 10.43 21.91 13.57 18.07 15.49L9.51002 19.77C3.75002 22.65 1.40002 20.29 4.28002 14.54L5.15002 12.81C5.37002 12.37 5.37002 11.64 5.15002 11.2L4.28002 9.46001C1.40002 3.71001 3.76002 1.35001 9.51002 4.23001Z" stroke="#292D32" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                  <path d="M5.44 12H10.84" stroke="#292D32" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </button>
            </div>
          </div>
        </form>
      </div>
    </div>
  );
}

CreateComment.propType = {
  onCommentChange: Proptypes.instanceOf(Function),
  onCreateCommentSubmit: Proptypes.instanceOf(Function),
  commentThread: Proptypes.string,
};

export default CreateComment;
